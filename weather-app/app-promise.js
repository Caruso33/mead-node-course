const yargs = require('yargs');
const axios = require('axios');

var argv = yargs
  .option({
    a: {
      demand: true,
      alias: 'address',
      describe: 'Address to fetch weather for',
      string: true
    }
  })
  .help()
  .alias("help", 'h')
  .argv;

var encodedAddress = encodeURIComponent(argv.address);
geocodeUrl = `https://maps.googleapis.com/maps/api/geocode/json?address= ${encodedAddress}`;

axios.get(geocodeUrl).then((response) => {
  if (response.data.status === 'ZERO_RESULTS'){
    throw new Error('Unable to find that address' );
  }

  var lat = response.data.results[0].geometry.location.lat;
  var lng = response.data.results[0].geometry.location.lng;
  var weatherURL = `https://api.darksky.net/forecast/${key}/${lat},${lng}`;
  console.log(response.data.results[0].formatted_address);
  return axios.get(weatherURL);//!!

}).then((response)=>{//chained promises (instead of nested like with callbacks)
  var temperature = response.data.currently.temperature;
  var apparentTemp = response.data.currently.apparentTemperature;

  console.log(`It's currently ${temperature}.`);
  console.log(`It feels like ${apparentTemp}.`);
}).catch((e) => {
  if (e.code === 'ENOTFOUND') console.log('Unable to connect to Google servers');
  else {
    console.log(e.message);
  }
});
